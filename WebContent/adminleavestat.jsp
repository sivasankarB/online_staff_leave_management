...
<body>
	<h2
		style="color: yellow; background-color: black; height: 60px; text-align: center; padding:0px;margin:0px">Online
		...
		while (rt.next()) {
			...
			if (hod.equals("Granted")) {
				...
				if(admin.equals("pending"))
				{
					...
					h = "Forward";
					r="Reject";
					out.println("<tr><td>" + username + "</td><td>" + typeofleave + "</td>  <td>" + dor + "</td>   <td>"+ leavefrom + "</td>   <td>" + leaveto + "</td>   <td>" + noofdays + "</td>   <td>"+ purpose+"</td>   <td><form action='adminapprove' method='get'><input name='leave_id' type='hidden' value='"+leave_id+"'> <input name='emp_id' type='hidden' value='" +emp_id+ "'><button type='submit' value='" + h + "'</button></form></td>  <td><form action='adminreject' method='get'><input name='leave_id' type='hidden' value='"+leave_id+"'> <input name='emp_id' type='hidden' value='" +emp_id+ "'><button type='submit' value='" + r + "'</button></form></td></tr>");
